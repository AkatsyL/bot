...
def encode(text: str, key: str, image: BytesIO) -> Image:
    """
    ...
    for i, char in enumerate(text):  # Перебираем символы в тексте.
        ...
        new_color = color & 0xF80000
        new_color |= (char & 0xE0) << 11

        # Вшиваем часть символа в последние 2 бита синего канала
        new_color |= (color & (0x3F << 10))
        new_color |= (char & 0x18) << 5

        # Вшиваем часть символа в последние 3 бита зеленого канала
        new_color |= (color & (0x1F << 3))
        new_color |= (char & 0x7)

        img.putpixel(pix, color_number_to_rgb(new_color))  # Изменяем пиксель на картинке
...
def decode(key: str, image: BytesIO) -> str:
    """
...
if __name__ == '__main__':
    if input('(encrypt or decrypt) e or d: ') == 'e':
        ...
        crypto_img.save(f'crypto-{img_name}.png')
    else:
        img_path = input('img path: ')
        private_key = input('key: ')
